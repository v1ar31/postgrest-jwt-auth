-- for create a database schema especially for auth information
-- also need the PostgreSQL extension pgcrypto.
create extension if not exists pgcrypto;

-- put things inside the basic_auth schema to hide
...
create constraint trigger ensure_user_role_exists after insert or update on basic_auth.users
  for each row
...
create trigger encrypt_pass before insert or update on basic_auth.users
  for each row
...
create or replace function request_password_reset(email text)
returns void
language plpgsql as $$
declare
  tok uuid;
...
create or replace function reset_password(email text, token uuid, pass text)
returns void
language plpgsql as $$
declare
  tok uuid;
...
create or replace function basic_auth.send_validation()
returns trigger
language plpgsql as $$
declare
  tok uuid;
...
create trigger send_validation after insert on basic_auth.users
  for each row
...
create or replace function basic_auth.clearance_for_role(u name)
returns void 
language plpgsql as $$
declare
  ok boolean;
begin
  ...
  if not ok then
    raise invalid_password using message = 'current user not member of role ' || u;
  end if;
end
$$ LANGUAGE plpgsql;
...
begin
  if tg_op = 'INSERT' then
    ...
    insert into basic_auth.users (role, pass, email, verified)
           values (new.role, new.pass, new.email, coalesce(new.verified, false));
...
create trigger update_users instead of insert or update or delete on users 
    for each row 
...
-- generating jwt (only access token)
...
create or replace function basic_auth.current_email()
returns text
language plpgsql as $$
begin
